...
from .models import ApplicantTestScore, Campus, PISTApplicant, Program, TestCenter, TestSession
...
class EligibilityService:
    ...
    def evaluate(*, program: Program, matric_marks: int, matric_total: int, fsc_marks: int, fsc_total: int, tests: list[dict]) -> EligibilityResult:
        ...
        required_percentage = float(program.eligibility_percentage)
        ...
        if required_test != 'Other' and required_test not in provided_types:
...
class TestSchedulingService:
    ...
    def assign(applicant: PISTApplicant, *, submitted_at=None):
        ...
        with transaction.atomic():
            ...
            applicant.save(update_fields=['test_date', 'reporting_time', 'test_venue', 'test_building', 'test_hall', 'status', 'updated_at'])
            return session
...
class ApplicationProcessingService:
    ...
    def process_external_application(*, validated_data: dict):
        ...
        applicant.roll_number = RollNumberService.generate(campus_code=campus.code, program_code=program.code)
        applicant.save(update_fields=['test_type', 'test_score', 'roll_number', 'status', 'updated_at'])
